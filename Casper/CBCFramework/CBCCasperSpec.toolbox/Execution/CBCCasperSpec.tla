...
RECURSIVE Dep(_)
Dep(msg) ==
  IF Cardinality(Justification(msg)) <= 1
...
Later(msg,msgs) == {m \in msgs: msg \in DepPlusSet(Justification(m))}
...
Estimates(state) == {Estimate(m): m \in (state \cup DepSet(state))}

\* Latest estimate(s) from a validator in a given set of messages.
LatestEsts(state) == Estimates(LatestMsgs(state))
LatestEstsFromFrom(n,state) == {Estimate(m): m \in LatestMsgsFrom(n,state)}
...
\*  IF msg = genesis
\*  THEN {gen_block}
\*  ELSE UNION{LatestEstsFrom(n,Justification(msg)): n \in Observed(Justification(msg))}
...
\* Last modified Wed Dec 18 18:42:01 EST 2019 by isaac

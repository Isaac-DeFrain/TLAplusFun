...
vars == <<faulty,scored_q,unscored_q,sent_msgs,equiv_msgs,estimates,states>>
...
Initialize(val) == [i \in 1..Cardinality(nodes) |-> val]
...
EquivocatedMsgs(n,msgs) == DepSet(msgs) \cap UnSeqSet(equiv_msgs[n])
...
\* Checks for existence of an e-clique with cumulative weight > 50% of total validator weight.
...
HonestReceivedMsgs(n) == {m \in ReceivedMsgs(n): m \notin UnSeqSet(equiv_msgs[n])}

\* A temporal property checking that finality can eventually be reached.
CheckSafetyOracle ==
  LET n == RandomElement(nodes \ GlobalFaultySet)
  IN  <>(\E v \in values : EcliqueEstimateSafety(v,HonestReceivedMsgs(n)))
...
Init ==
  /\ ThresholdCheck
  /\ NodeWeightLen
  /\ dags       = Initialize(<<genesis>>)
  /\ faulty     = Initialize({})
  /\ scored_q   = Initialize(<<ScoredMsg(genesis,TotalWeight)>>)
  /\ unscored_q = Initialize(<<>>)
  /\ sent_msgs  = Initialize(<<>>)
  /\ equiv_msgs = Initialize({})
  /\ estimates  = Initialize(values)
  /\ states     = Initialize({genesis})

----------------------------------------------------------------------------
\* Updates
\* A validator can update their set of valid estimates.
Update_Estimates(n) ==
  /\ estimates' = [estimates EXCEPT ![n] = GHOST(states[n])]
  /\ UNCHANGED <<dags,faulty,scored_q,unscored_q,sent_msgs,equiv_msgs,states>>

\* A validator can score unscored estimates and update their scores.
Update_Scores(msg,n,rec) ==
  /\ scored_q'   = [scored_q EXCEPT ![n] =
     UpdateScores(scored_q[n],states[n]) \o ScoreUnscored(unscored_q[n],states[n])]
  /\ unscored_q' = [unscored_q EXCEPT ![n] = <<>>]
  /\ UNCHANGED <<dags,faulty,sent_msgs,equiv_msgs,estimates,states>>
...
Send_Honest ==
  ...
  /\ LET v == RandomElement({n \in (nodes \ GlobalFaultySet): estimates[n] # {}}) \* honest validator with valid estimate
     ...
     IN  /\ SendMsg(Msg(e,v,states[v]),v,nodes)
         /\ UNCHANGED <<dags,faulty,equiv_msgs,estimates>>
...
Send_Drop ==
  ...
  /\ LET v == RandomElement({n \in nodes : estimates[n] # {}})
     ...
     IN  /\ sent_msgs'  = [sent_msgs EXCEPT ![v] = sent_msgs[v] \o <<Msg(e,v,states[v])>>] 
         ...
         /\ UNCHANGED <<dags,faulty,unscored_q,equiv_msgs,estimates>>
...
Send_Equiv_Est ==
  ...
  /\ LET v == RandomElement({n \in nodes: Cardinality(estimates[n]) > 1})
     ...
     IN /\ SendMsg(Msg(e1,v,states[v]),v,N1)
        /\ SendMsg(Msg(e2,v,states[v]),v,N2)
        /\ UNCHANGED <<dags,faulty,equiv_msgs>>
...
Send_Equiv_Just ==
  ...
  /\ LET v == RandomElement({n \in nodes: Cardinality(states[n]) > 1})
     ...
     IN /\ SendMsg(Msg(e,v,j1),v,N1)
        /\ SendMsg(Msg(e,v,j2),v,N2)
        /\ UNCHANGED <<dags,faulty,equiv_msgs>>
...
Send_Equiv_Both ==
  ...
  /\ LET v == RandomElement({n \in nodes: Cardinality(states[n]) > 1})
        ...
        j1 == RandomElement(SUBSET(states[v]))
        j2 == RandomElement({j \in SUBSET(states[v]): j # j1})
     IN /\ SendMsg(Msg(e1,v,j1),v,N1)
        /\ SendMsg(Msg(e2,v,j2),v,N2)
        /\ UNCHANGED <<dags,faulty,equiv_msgs>>
...
(* to faulty set and the equivocated messages to equiv_msgs *)
HandleEquiv ==
  ...
  /\ LET n == RandomElement({v \in nodes: CheckDepsForEquiv(ReceivedMsgs(v))})
     ...
     IN  /\ faulty'     = [UnionEach(faulty,{e})   EXCEPT ![e] = faulty[e]]
         /\ equiv_msgs' = [UnionEach(equiv_msgs,E) EXCEPT ![e] = equiv_msgs[e]]
         /\ UNCHANGED <<dags,scored_q,unscored_q,sent_msgs,estimates,states>>
...
\* Last modified Sat Dec 14 13:09:37 EST 2019 by isaac

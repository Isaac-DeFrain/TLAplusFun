...
\*genesis,     \* genesis message - model value
  gen_block,   \* estimate of genesis message - model value
  gen_sender   \* sender of genesis message - model value
...
vars == <<dags,faulty,scored_q,unscored_q,sent_msgs,equiv_pairs,estimates,states>>
...
\* The genesis message.
genesis == Msg(gen_block,gen_sender,{})
...
RECURSIVE Dep(_)
Dep(msg) ==
  IF Cardinality(Justification(msg)) < 2
  THEN Justification(msg)
  ELSE Justification(msg)
       \cup UNION{Dep(m): m \in Justification(msg)}

\* Dependencies plus message.
DepPlus(msg) == {msg} \cup Dep(msg)

\* Set of dependencies of all the messages in a set of messages.
DepSet(msgs)     == UNION{Dep(m) : m \in msgs}
DepPlusSet(msgs) == UNION{DepPlus(m) : m \in msgs}
...
\* A later message is not contained in the justification of any other message.
Later(msg,state) == {m \in state: msg \in DepPlusSet(Justification(m))}

\* Latest messages in a set of messages, i.e. no message in the set is later.
LatestMsgs(state) == {m \in state: Later(m,state) = {}}

\* Latest message(s) from a validator in a given set of messages.
LatestMsgsFrom(n,msgs) ==
  {m \in LatestMsgs(msgs): Sender(m) = n}

\* Set of estimates in a given set of messages.
Estimates(msgs) == {Estimate(m): m \in msgs}

AllEstimates(state) == {Estimate(m): m \in DepPlusSet(state)}

\* Latest estimates.
LatestEsts(state) == Estimates(LatestMsgs(state))

\* Latest estimate(s) from a validator in a given set of messages.
LatestEstsFrom(n,state) == {Estimate(m): m \in LatestMsgsFrom(n,state)}

\* Justifications of a set of messages.
Justifications(state) == UNION{Justification(m): m \in state}
...
CheckDepsForEquiv(state) ==
  LET deps == DepPlusSet(state)
  IN  \E m1,m2 \in deps : Equivocation(m1,m2)

EquivPairsInDeps(state) ==
  IF ~CheckDepsForEquiv(state)
  THEN {}
  ELSE {<<m1,m2>> \in (DepPlusSet(state) \ {genesis}) \X (DepPlusSet(state) \ {genesis}): Equivocation(m1,m2)}

\* A validator is faulty if it sends equivocating messages.
\* Checks if a validator equivocates in a given set of messages.
FaultyNode(n,state) ==
  /\ \E m1 \in (DepPlusSet(state) \ {genesis}) : 
     /\ \E m2 \in (DepSet(state) \ {genesis}) :
        /\ Sender(m1) = n
        /\ Equivocation(m1,m2)

\* Set of faulty validators in an observed set of messages.
FaultyNodes(state) == {n \in nodes : FaultyNode(n,state)}

\* Messages from equivocating validators in a given set of messages.
EquivocatedMsgs(n,state) == DepPlusSet(state) \cap UnSeqSet(equiv_pairs[n])

\* Checks existence of equivocated messages received by the given validator.
EquivReceived(n) == \E <<m1,m2>> \in Pairs(DepPlusSet(ReceivedMsgs(n))) : Equivocation(m1,m2)
...
HonestMsgs(n,state) == state \ EquivocatedMsgs(n,state)
\*HonestMsgs(n,state) == DepPlusSet(state) \ EquivocatedMsgs(n,state)

\* Set of latest honest messages received by a validator.
LatestHonestMsgs(n,msgs) == {m \in HonestMsgs(n,msgs): m \in LatestMsgsFrom(n,msgs)}
...
Agreeing(n1,n2,estimate,msgs) ==
  LET n1_latest_msg == Pick(LatestMsgsFrom(n1,msgs))
      n2_latest_msg == Pick(LatestMsgsFrom(n2,Justification(n1_latest_msg)))
  IN  /\ Cardinality(LatestMsgsFrom(n1, msgs)) = 1
      /\ Cardinality(LatestMsgsFrom(n2,Justification(n1_latest_msg))) = 1
...
Disagreeing(n1,n2,estimate,msgs) == 
  /\ Cardinality(LatestMsgsFrom(n1,msgs)) = 1
  /\ LET n1_latest_msg == Pick(LatestMsgsFrom(n1,msgs))
     IN  /\ Cardinality(LatestMsgsFrom(n2,Justification(n1_latest_msg))) = 1
         /\ LET n2_latest_msg == Pick(LatestMsgsFrom(n2,Justification(n1_latest_msg)))
...
(*************)
\*TODO: make precise
\* GHOST fork choice rule - latest honest message driven
\* output should be ranked set of tips (messages) from which to build dag
GHOST(state) ==
  IF state = {genesis}
  THEN genesis
  ELSE UNION{LatestMsgs(m): m \in state}
...
PrevMsg(msg) ==
  IF msg = genesis
  THEN {genesis}
  ELSE UNION(LatestMsgs(Justification(msg)))

\* Previous estimates.
PrevEst(msg) ==
  IF msg = genesis
  THEN {gen_block}
  ELSE UNION(LatestEsts(Justification(msg)))
...
Supporters(est,state) ==
  IF state = {genesis} \/ est \notin AllEstimates(state)
  THEN {}
  ELSE LET m == Pick(state \ {genesis})
       IN  IF est \in AllEstimates({m})
...
\* Last modified Thu Dec 19 20:11:53 EST 2019 by isaac

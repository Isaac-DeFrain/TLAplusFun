...
CONSTANT
  Nodes,          \* set of participating nodes
                  \* Nodes \subseteq [id : 1..n, bootstrap : 1..n, status : PossibleStatuses ]
...
\*Fold(f,a,b) = f(a,b)
\*Fold(f(_,_),a,s) = IF i = Len(s) THEN Fold(f,f(a,s[i]),s[i-1]) ELSE Fold(f,f(a,s[i]),s[i-1])
...
TypeOK == \A n \in nodes :
  /\ nStatus[n] \in PossibleStatuses
  /\ HoldsMessageType(nInMsg[n]) 
  /\ HoldsMessageType(nOutStreamMsg[n])
  /\ nOutMsg[n] \in ([status : SendingStatus, packet : Packets] \cup {None})
...
EventuallyAllNodesAreRunning == <>[][\A n \in nodes : nStatus[n] = "running"]_vars
...
\* Last modified Fri Nov 22 12:00:37 EST 2019 by isaac
